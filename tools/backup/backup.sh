...
function cloud_copy() {
  ...
  gcp)
    ...
    ;;
  azure)
    az storage container create -n "$BUCKET-$database" --account-name $ACCOUNT_NAME --subscription $SUBSCRIPTION
    az storage blob upload --container-name "$BUCKET-$database" \
                       --file "$backup_path" \
